...
- Final focused independent review is closed; the integrated local suite has 1286 passing tests.
...
- Latest full CI mutation run (35058666946, 074996d) has 672 unresolved survivors and 8 timeouts (down from 5,326 on the first measured run); the strengthened CI outcome gate fails explicitly until they are resolved. Mutants proven to change nothing observable are recorded with their proof in docs/v2/MUTATION_EXEMPTIONS.md; the gate does not read that file and fails on them too.
